define(['underscore', 'resthub', 'und!template/test/testList'], 
function(_, Resthub, template) {
	var ListView = Resthub.View.extend({
		...
		initialize : function(options) {
			...
			_self.render();
			// $.ajax({
				// url : window.contextBase + "/api/test",
		...
		render : function(data) {
			var _self = this;
			_self.$el.html(_self.template({tests:[]}));
			// if (data.content){//pagination goes here
				// if (data.content.length===0 && data.totalPages!==0){
					// window.approuter.navigate("/test?page=" + data.totalPages,{trigger:"true"});
				// }else{
					...
					// }else{
						...
						// };
					// }
					// _self.$el.html(_self.template(tests:data.content}));
					// _.defer(function() {
				        // _self.$el.find('.pagination').bootstrapPaginator(options);
				     // });
				// }
			// }else{// no pagination
				// _self.$el.html(_self.template(tests:data}));
			// }
		...
		deleteItem : function(e){
			var _self = this;
			var testId = $(e.target).closest('td').attr('data-testId');
			
			$.ajax({
				type:"DELETE",
				url:window.contextBase + "/api/test/"+ testId
			}).done(function(){
				window.globalNotify({
					type:"success",
					htmlContent:"删除成功！",
